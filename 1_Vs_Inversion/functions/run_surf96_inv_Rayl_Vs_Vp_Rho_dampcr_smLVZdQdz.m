function [finalmod,cpre,vs_std,vp_std,rho_std] = run_surf96_inv_Rayl_Vs_Vp_Rho_dampcr_smLVZdQdz(cobs,cstd,periods,startmod,discs,eps_data,eps_H,eps_J,eps_F,eps_vpvs,eps_rhovs,z_dampbot,nit,nit_recalc_c,vp_vs,rho_vs,nmode,dQdz,varargin)
...
elseif length(varargin)==6
    ...
    fac = varargin{3};
    alpha = varargin{4}; 
    zmin_dQdz = varargin{5}; 
    zmax_dQdz = varargin{6}; 
end
...
z_brks_LVZ = sort(discs_LVZ);
J00 = smooth_LVZ_constraint_dQdz(J00, z, z_brks_LVZ, fac, alpha, dQdz, zmin_dQdz, zmax_dQdz);
F00 = smooth_LVZ_constraint_dQdz(F00, z, z_brks_LVZ, fac, alpha, dQdz, zmin_dQdz, zmax_dQdz);

z_brks = [z_brks; z_brks_LVZ];
...
H00(ind_h2o,ind_h2o) = H00(ind_h2o,ind_h2o)*1e9;
h0(ind_h2o) = h0(ind_h2o)*1e9;
% Damp crust to starting model
ind_cr = find(startmod(:,2)<7);
H00(ind_cr,ind_cr) = H00(ind_cr,ind_cr)*1e9;
h0(ind_cr) = h0(ind_cr)*1e9;
% % Undamp (relax) 1st derivative smoothing below bottom disc.
% ind_relaxJ = find(z>max(z_brks));
% ind_relaxJ = ind_relaxJ(2:end);
% J00(ind_relaxJ,:) = J00(ind_relaxJ,:)/10;
% j0(ind_relaxJ) = j0(ind_relaxJ)/10;
