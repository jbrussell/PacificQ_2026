...
if size(model,2)>4 % if Qp and Qs proivded, calculate the Q-corrected kernels

    kernel = calc_anelastic_kernel96(model, vec_T, wavetype, ifnorm, ifplot, nmode, fref);
    dcdvs=kernel.dcdvs;
    dcdvp=kernel.dcdvp;
    dudvs=kernel.dudvs;
    dudvp=kernel.dudvp;
    dcdrho=kernel.dcdrho;
    dudrho=kernel.dudrho;
    z_mid=kernel.z;

else % Otherwise, calculate non-Q-corrected kernels
