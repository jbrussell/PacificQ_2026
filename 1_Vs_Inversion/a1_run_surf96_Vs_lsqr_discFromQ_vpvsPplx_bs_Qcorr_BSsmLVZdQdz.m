...
% prior_type = 'uniform';
% age_myr = 43;
% matnameQ = './bayesian_mcmc_Qspline_zknot_112s/YoungORCA_uniform_Qmu_bayesian_Nspline12.mat';
% nmode = 0;
% fref = 1/65; % Hz
% alpha = 1; % Add curvature to smoothing strength reduction profile
% fac = 0.25^(1/alpha); % multiplier to reduce smoothing constraints by
% fac_std = 0.1; %fac*0.25; % standard deviation for perturbing fac
% zmin_dQdz = 30; % [km] min depth of Q gradient to consider
% zmax_dQdz = 280; % [km] max depth of Q gradient to consider
% par.mult_std_pert = 2; % factor to multiply std for phv perturbations
...
% prior_type = 'uniform';
% age_myr = 70;
% matnameQ = './bayesian_mcmc_Qspline_zknot_112s/NoMelt_uniform_Qmu_bayesian_Nspline12.mat';
% nmode = 0;
% fref = 1/65; % Hz
% alpha = 1; % Add curvature to smoothing strength reduction profile
% fac = 0.25^(1/alpha); % factor to reduce smoothing constraints by
% fac_std = 0.1; %fac*0.2; % standard deviation for perturbing fac
% zmin_dQdz = 30; % [km] min depth of Q gradient to consider
% zmax_dQdz = 280; % [km] max depth of Q gradient to consider
% par.mult_std_pert = 2; % factor to multiply std for phv perturbations
...
prior_type = 'uniform';
age_myr = 89;
matnameQ = './bayesian_mcmc_Qspline_zknot_112s/OldORCA_uniform_Qmu_bayesian_Nspline12.mat';
nmode = 0;
fref = 1/65; % Hz
alpha = 1; % Add curvature to smoothing strength reduction profile
fac = 0.25^(1/alpha); % factor to reduce smoothing constraints by
...
Nbs = 1000; %1000; % number of bootstraps
...
outdir = ['./lsqr_kernel_Vs_vpvsPerplex_Qcorr',num2str(1/fref),'s_bootstrap_BSsmLVZdQdz/'];
...
z_q = bayesianQ.z_int;
% if strcmp(PROJ,'NoMelt')
%     qmu_inv(z_q>300) = 1/1000;
% end
...
[refmod, discs, zlays] = card2mod(CARD,zbot+20); % true model
% Add Qp and Qs columns to refmod
qmu_inv_mod = interp1(z_q,qmu_inv,zlays,'linear','extrap');
qmu_inv_mod = 0.5*(qmu_inv_mod(1:end-1)+qmu_inv_mod(2:end));
qkap_inv_mod = 1./(5000) * ones(size(qmu_inv_mod));
refmod = [refmod, 1./qkap_inv_mod(:), 1./qmu_inv_mod(:)];
...
subplot(1,3,1);
box on; hold on;
temp = read_model_card(CARD);
h = plotlayermods(startmod(:,1),startmod(:,3),'-b');
h.LineWidth = 2;
plot(temp.vsv/1000,temp.z,'-r','linewidth',2);
xlabel('Vs (km/s)');
ylabel('Depth (km)');
title('Starting Model');
set(gca,'FontSize',18,'linewidth',1.5);
ylim([0 sum(startmod(:,1))]);

subplot(1,3,2);
box on; hold on;
h = plotlayermods(startmod(:,1),1./startmod(:,6),'-b');
h.LineWidth = 2;
plot(qmu_inv,z_q,'-r','linewidth',2);
xlabel('Q_s^{-1}');
ylabel('Depth (km)');
...
Qinv = 1./startmod(:,6);
alph = 0.2; % ranges from 0.1-0.3
dVs_anel = 1 - Qinv/(2*tan(pi*alph/2));

subplot(1,3,3);
box on; hold on;
h = plotlayermods(startmod(:,1),dVs_anel,'-b');
h.LineWidth = 2;
plot(1 - qmu_inv/(2*tan(pi*alph/2)),z_q,'-r','linewidth',2);
xlabel('Vs / Vs_{anh}');
ylabel('Depth (km)');
set(gca,'FontSize',18,'linewidth',1.5);
ylim([0 sum(startmod(:,1))]);
...
for ibs = 1:Nbs
    ...
    if ibs==1
        ...
        [ cobs_pert ] = perturb_data( cobs, cstd * par.mult_std_pert  );
        [ fac_pert ] = perturb_data( fac, fac_std  );
        if fac_pert < 0
            fac_pert = 0;
        end
    end
    ...
    [finalmod_smooth, cpre_smooth, vs_std_smooth] = run_surf96_inv_Rayl_Vs_Vp_Rho_dampcr(cobs_pert,cstd,periods,startmod,discs,eps_data,eps_H,eps_J,eps_F,eps_vpvs,eps_rhovs,z_dampbot,nit,nit_recalc_c,par.vp_vs,par.rho_vs,nmode,fref);
...
%     [finalmod_disc, cpre_disc, vs_std_disc] = run_surf96_inv_Rayl_Vs_Vp_Rho_dampcr_relaxbot(cobs_pert,cstd,periods,startmod,discs_all,eps_data,eps_H,eps_J,eps_F,eps_vpvs,eps_rhovs,z_dampbot,nit,nit_recalc_c,par.vp_vs,par.rho_vs,nmode,fref);
    
    % Now solve for model with rougher LVZ
    discs_LVZ = zdisc_Q(:);
    dQdz = [abs(dqmu_inv_dr), z_q];
    [finalmod_disc, cpre_disc, vs_std_disc] = run_surf96_inv_Rayl_Vs_Vp_Rho_dampcr_smLVZdQdz(cobs_pert,cstd,periods,startmod,discs,eps_data,eps_H,eps_J,eps_F,eps_vpvs,eps_rhovs,z_dampbot,nit,nit_recalc_c,par.vp_vs,par.rho_vs,nmode,dQdz,fref,discs_LVZ,fac_pert,alpha);
...
%     [card_disc] = mod2card_discs(finalmod_disc,discs_all,card_ref);
    [card_disc] = mod2card_discs(finalmod_disc,discs_save,card_ref);
...
clr = jet(Npers);
lgd = {};
[dcdvs, dcdvp, dudvs, dudvp, zkern, dcdrho, dudrho] = calc_kernel96(finalmod_disc, periods, 'R', 1, 0,nmode,fref);
...
% Get various inversion parameters
lsqrinv.param.path2perlextab_vs = path2perlextab_vs;
lsqrinv.param.path2perlextab_vp = path2perlextab_vp;
lsqrinv.param.path2perlextab_rho = path2perlextab_rho;
lsqrinv.param.Tp_C = Tp_C; 
lsqrinv.param.modeltype = modeltype;
lsqrinv.param.matnameQ = matnameQ;
lsqrinv.par.nmode = nmode;
lsqrinv.par.fref = fref; % Hz
lsqrinv.par.alpha = alpha; % Add curvature to smoothing strength reduction profile
lsqrinv.par.fac = fac; % factor to reduce smoothing constraints by
lsqrinv.par.fac_std = fac_std; % standard deviation for perturbing fac
lsqrinv.par.zmin_dQdz = zmin_dQdz; % [km] min depth of Q gradient to consider
lsqrinv.par.zmax_dQdz = zmax_dQdz; % [km] max depth of Q gradient to consider
lsqrinv.par.eps_data = eps_data; % data fit
lsqrinv.par.eps_H = eps_H; % norm damping
lsqrinv.par.eps_J = eps_J; %0*0.001; % first derivative smoothing
lsqrinv.par.eps_F = eps_F; %1; % second derivative smoothing
lsqrinv.par.eps_vpvs = eps_vpvs; % enforce Vp/Vs
lsqrinv.par.eps_rhovs = eps_rhovs; % enforce Rho/Vs
lsqrinv.par.z_dampbot = z_dampbot; % [km] damp to starting model below this depth
lsqrinv.par.zbot = zbot; % maximum depth of entire model
lsqrinv.par.nit = nit; % total number of iterations
lsqrinv.par.nit_recalc_c = nit_recalc_c; % number of iterations after which to recalculate phase velocity and kernels
lsqrinv.par.Nbs = Nbs; % number of bootstraps
lsqrinv.par.chi2_thresh = chi2_thresh; %1.5; % Ignore models with chi^2 misfit greater than this
lsqrinv.par.errmultfac = errmultfac; % factor to multiply errors by
