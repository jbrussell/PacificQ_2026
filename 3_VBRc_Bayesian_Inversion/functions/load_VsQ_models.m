function [bootstrap_Vs,bootstrap_Q,discs] = load_VsQ_models(PROJ,path2Vs,path2Q,type,fignum)
...
nf89_vs = load_NF89_vs('./NF89/');
% ages = cell2mat({nf89_vs.age}');
if strcmp(PROJ,'Young ORCA')
    nf89_vs = nf89_vs(3);
elseif strcmp(PROJ,'NoMelt')
    nf89_vs = nf89_vs(4);
elseif strcmp(PROJ,'Old ORCA')
    nf89_vs = nf89_vs(5);
end
bootstrap_Vs.xi_NF89 = interp1(nf89_vs.z,nf89_vs.xi,bootstrap_Vs.bayesian.z_int);
inan = find(isnan(bootstrap_Vs.xi_NF89));
inan_shallow = inan(find(bootstrap_Vs.bayesian.z_int(inan)<20));
inan_deep = inan(find(bootstrap_Vs.bayesian.z_int(inan)>20));
bootstrap_Vs.xi_NF89(inan_shallow) = nf89_vs.xi(1);
bootstrap_Vs.xi_NF89(inan_deep) = nf89_vs.xi(end);
% Isotropic Vs
bootstrap_Vs.bayesian.post.vsh_NF89_l68 = bootstrap_Vs.bayesian.post.vs_l68 .* sqrt(bootstrap_Vs.xi_NF89);
bootstrap_Vs.bayesian.post.vsh_NF89_u68 = bootstrap_Vs.bayesian.post.vs_u68 .* sqrt(bootstrap_Vs.xi_NF89);
bootstrap_Vs.bayesian.post.vsh_NF89_med = bootstrap_Vs.bayesian.post.vs_med .* sqrt(bootstrap_Vs.xi_NF89);
bootstrap_Vs.bayesian.post.vsvoigt_NF89_l68 = sqrt((2*bootstrap_Vs.bayesian.post.vs_l68.^2 + bootstrap_Vs.bayesian.post.vsh_NF89_l68.^2)/3);
bootstrap_Vs.bayesian.post.vsvoigt_NF89_u68 = sqrt((2*bootstrap_Vs.bayesian.post.vs_u68.^2 + bootstrap_Vs.bayesian.post.vsh_NF89_u68.^2)/3);
bootstrap_Vs.bayesian.post.vsvoigt_NF89_med = sqrt((2*bootstrap_Vs.bayesian.post.vs_med.^2 + bootstrap_Vs.bayesian.post.vsh_NF89_med.^2)/3);
