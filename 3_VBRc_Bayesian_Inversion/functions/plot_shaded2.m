function [ h ] = plot_shaded( x_l, x_u, y, opt, clr, alph )
...
if strcmp(opt,'x')==1
    h = patch([x_l fliplr(x_u)],[y fliplr(y)],clr,'linestyle','none'); hold on;
    set(h, 'FaceAlpha', alph);
    plot(x_l,y,'-','color',clr,'linewidth',1);
    plot(x_u,y,'-','color',clr,'linewidth',1);
elseif strcmp(opt,'y')==1
    h = patch([y fliplr(y)],[x_l fliplr(x_u)],clr,'linestyle','none'); hold on;
    plot(y,x_l,'-','color',clr,'linewidth',1);
    plot(y,x_u,'-','color',clr,'linewidth',1);
    set(h, 'FaceAlpha', alph);
end
