function [solution] = solve_for_stable_melt_fraction_meltfree(T_K,P_GPa,C_h2o_solid,D_h2o,C_co2_solid,D_co2,solidus_str)
% Using equations in appendix of Blatter et al. (2022), solve for
% thermodynamically stable melt fraction given temperature, pressure, and
% bulk volatile content.
%
% INPUTS
...
% Range over which to evaluate melt fraction, F
F_test = [0:0.0001:1]';
...
%     h = F_test(:) - ((T_K(iz)-Tsolidus_K(:))./(Solidus.Tlherz_dry(:)-Solidus.Tsol_dry(:))).^1.5;
    ...
    solution.Cs_H2O_0(iz) = Cs_H2O; % Initial H2O concentration
    solution.Cs_CO2_0(iz) = Cs_CO2; % Initial CO2 concentration
    solution.Cs_H2O(iz) = D_h2o*solution.Cf_H2O(iz); % H2O concentration left over in solid
    solution.Cs_CO2(iz) = D_co2*solution.Cf_CO2(iz); % CO2 concentration left over in solid
    solution.Cs_H2O_ppm(iz) = solution.Cs_H2O(iz)*1e4; % H2O concentration left over in solid (ppm)
    solution.Cs_CO2_ppm(iz) = solution.Cs_CO2(iz)*1e4; % CO2 concentration left over in solid (ppm)
