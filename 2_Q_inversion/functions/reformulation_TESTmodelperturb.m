...
function mest = reformulation_TESTmodelperturb(Lphv,Lstd,Rphv,Rstd,forward,frech_T,frech_S,periods,card)
% Lphv = obs.Lphv;
% Rphv = obs.Rphv;
% Lstd = obs.Lstd;
% Rstd = obs.Rstd;
...
for ip = 1:length(frech_T)
    GG(ip,1:length(tindrad)) = frech_T(ip).vsv(tindrad).*dr*1000;
    GG(ip,length(tindrad)+1:length(tindrad)*2) = frech_T(ip).vsh(tindrad).*dr*1000;
    %GG(ip,length(tindrad)*2+1:length(tindrad)*3) dc/dVPV = 0;
    %GG(ip,length(tindrad)*3+1:length(tindrad)*4) dc/dVPH = 0;
    %GG(ip,length(tindrad)*4+1:length(tindrad)*5) dc/dETA = 0;
end
...
for ip = 1:length(frech_S)
    GG(ip+length(frech_T),1:length(sindrad)) = frech_S(ip).vsv(sindrad).*dr*1000;
    %GG(ip+length(frech_T),length(sindrad)+1:length(sindrad)*2) = frech_S(ip).vsh(sindrad); % dc/dVSH = 0
    GG(ip+length(frech_T),length(sindrad)*2+1:length(sindrad)*3) = frech_S(ip).vpv(sindrad).*dr*1000;
    GG(ip+length(frech_T),length(sindrad)*3+1:length(sindrad)*4) = frech_S(ip).vph(sindrad).*dr*1000;
    GG(ip+length(frech_T),length(sindrad)*4+1:length(sindrad)*5) = frech_S(ip).eta(sindrad).*dr*1000;    
end
