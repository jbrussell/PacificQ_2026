function [phV,grV,phVq,Q] = readMINEOS_qfile_2(qfile,periods,mode)
...
for ii = 1:length(mode)
    ...
    % desired frequencies
    freq_want = 1./periods(ii);

    % interpolate to get velocities
    phV(ii) = interp1(freq_all,phV_all,freq_want);
    grV(ii) = interp1(freq_all,grV_all,freq_want);

    phVq(ii) = interp1(1./T_all_q,phV_all_q,freq_want);
